$(function(){
    ...
    var templates = {
        ...
        'uf_nome': {'label': 'DESCR', 'template': '<p>{{ALFA1}} - {{DESCR}}</p>', 'minLength': 1},
        'seguradoras': {'label': 'DESCR', 'template': '<p>{{COD1}} - {{DESCR}}</p>', 'minLength': 1},
        'seguradora': {'label': 'FANTASIA', 'template': '<p>{{CNPJ}} - {{FANTASIA}}</p>', 'minLength': 3},
        'cfop': {'label': 'DESCR', 'template': '<p>{{COD1}} - {{DESCR}}</p>', 'minLength':2},
        'produtos': {'label': 'DESCRICAO', 'template': '<p>{{CODIGO}} - {{DESCRICAO}}</p>', 'minLength':1},
        'veiculos': {'label': 'VEICULO', 'template': '<p>{{PLACA}} - {{VEICULO}}</p>', 'minLength':1},
        'cst': {'label': 'DESCR', 'template': '<p>{{COD1}} - {{DESCR}}</p>', 'minLength':1},
        'operador': {'label': 'NOME', 'template': '<p>{{CODIGO}} - {{NOME}}</p>', 'minLength':1},
        'formapagto': {'label': 'NOME', 'template': '<p>{{CODIGO}} - {{NOME}}</p>', 'minLength':1},
        'rotas': {'label': 'DESCRICAO', 'template': '<p>{{DESCRICAO}}</p>', 'minLength':1},
    };
    ...
    $.fn.setComplete = function(valueKey, func, execEvent){
        //LIMPANDO OS CAMPOS
        this[0].complete.hidden.val('');
        this[0].complete.input.val(valueKey);
        ...
        $(this).on('typeahead:render', function(e, data){
            ...
            if(typeof func == 'function') func(keyVal, labelVal, e, data);
            if(execEvent){
                let eventT = jQuery.Event('get:selected', {'response':data});
                $(e.currentTarget).trigger(eventT);
            }

            if(this.complete.input.val() == valueKey){
    ...
    class AutoComplete {
        constructor(options) {
            ...
            this.createHidden();
            this.options.hidden[0].complete = this.options;
        ...
        createBlood() {
            ...
            let url = 'auto/' + String(opt.table).toUpperCase() + '/';
        ...
        start(blood) {
            ...
            $(ipt).on('typeahead:selected', function(event, data){
                ...
                opt.hidden.attr('nVal', data[opt.key]);
        ...
        createHidden() {
            ...
            $.each(ipt[0].attributes, function(){
                if(this.name != 'id' && this.name != 'disabled' && this.name != 'required'){
                    hidden.setAttribute(this.name, this.value);
                }
            });
