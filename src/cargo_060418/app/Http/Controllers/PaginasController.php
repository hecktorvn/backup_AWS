...
class PaginasController extends Controller
{
    ...
    public function viewCte(Request $req, $chave){
        if(file_exists(storage_path() . "/CTe/xml/{$chave}-cte.xml")){
            $content = file_get_contents(storage_path() . "/CTe/xml/{$chave}-cte.xml");
            return response( $content )->header('Content-type', 'application/xml');
        } else {
            return response()->view('error', ['title'=>'404', 'msg_error'=>'Arquivo <b>CT-e</b> não encontrado!'], 404);
        }
    }
    ...
    public function lista(Request $req, $name) {
        ...
        if (View::exists($view)) {
            ...
            return self::returnView($view, ['name'=>$name, 'header'=>$this::getHeaderList($name), 'ordem'=>Tabelas::$ordem[ strtoupper($name) ], 'pk'=>$pk], 'dataList');
        } else {
