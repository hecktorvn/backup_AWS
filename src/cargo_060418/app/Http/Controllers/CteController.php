...
class CteController extends Controller
{
    ...
    public function gerar($filial, $codigo){
        $chave = montaChave(
            $this->conf['cUF'], date('y', strtotime($dhEmi)), date('m', strtotime($dhEmi)), $this->conf['cnpj'], $this->tools->model(), '1', $numeroCTE, '1', $numeroCTE
        );
    ...
    public function enviar(){
        ...
        if ($std->protCTe->infProt->cStat == 100) {
            ...
            //adicionar protocolo
            $response = $tools->sefazConsultaChave($this->chave);
