...
class CteController extends Controller
{
    ...
    public function gerar($filial, $codigo){
        $chave = montaChave(
            $this->conf['cUF'], date('y', strtotime($dhEmi)), date('m', strtotime($dhEmi)),
            $this->conf['cnpj'], $this->tools->model(), '1', $numeroCTE, '1', $numeroCTE
        );
    }
    ...
    public function enviar(){
        ...
        $filename_send = storage_path() . "/CTe/retorno/{$chave}-env.xml";
        file_put_contents($filename_send, $res);
        ...
        if ($std->protCTe->infProt->cStat == 100) {
            ...
            $filename_con = storage_path() . "/CTe/retorno/{$chave}-con.xml";
            file_put_contents($filename_con, $response);
            ...
            $filename = storage_path() . "/CTe/aprovadas/{$chave}-protcte.xml";
    ...
    public function montaCTe($filial, $codigo){
        ...
        */
        ...
        if(empty($_ctrc->VALOR_RECEBER)) $vPrest->vRec = floatval(Format::format($_ctrc->VALOR_FRETE, 'numeric')); // Valor a receber
        else $vPrest->vRec = floatval(Format::format($_ctrc->VALOR_RECEBER, 'numeric')); // Valor a receber
        ...
        if($_emitente->OPTANTE_SIMPLES == 1 || is_null($_ctrc->SITUACAO_TRIBUTARIA)):
            ...
            $icms->vTotTrib = floatval( number_format($xTtIMP, 2) );
        endif;
        ...
        $cte->montaCTe();
        $chave = $cte->chCTe;
        $filename = storage_path() . "/CTe/xml/{$chave}-cte.xml";
        $this->xml = $cte->getXML();
