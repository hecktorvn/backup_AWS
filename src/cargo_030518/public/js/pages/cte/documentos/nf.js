function startNfDocumentos(box, nameObj) {
    ...
    btn_incluir.off('click').on('click', function() {
        ...
        if (!valid) return false;
            ...
            $[nameObj].push( $.toObject(form.serializeArray()) );
            form.flush();
        ...
        if (Object.keys($[nameObj]).length > 0) {
            ...
            $.each($.toObject($[nameObj]), function(iO, vO) {
                ...
                $.each(campos, function(iC, vC) {
                    if(setTypes) data.push( $.format(vO[vC], types[vC]) );
    ...
    $[nameObj + '_methods'] = {};
    $[nameObj + '_methods'].drawTable = drawTable;
    $[nameObj + '_methods'].startEvent = function(name, value){
