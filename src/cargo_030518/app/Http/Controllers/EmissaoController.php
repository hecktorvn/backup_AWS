...
class EmissaoController extends Controller
{
    ...
    public function gravarManifesto(Request $req){
        ...
        try{
            ...
            $data_update = [
                ...
                'TOTAL' => number_format($req->TOTAL_FRETE, 2, '.', ''),
                'SITUACAO' => 0,
    ...
    public function getManifesto(Request $req, $filial=null, $codigo=null){
        if($req->isMethod('post')){
            $filial = $filial ?? $req->filial;
            $codigo = $codigo ?? $req->mdfe;
            ...
            return ['mdfe'=>$manifesto, 'seguro'=>$seguro, 'cte'=>$cte];
        } else {
            // CAPTURANDO O MANIFESTO
            $manifesto = DB::table('MANIFESTO')->where(['FILIAL'=>$filial, 'MANIFESTO'=>$codigo])->get()->toArray();
            $manifesto = $manifesto[0] ?? [];

            $data = ['filial'=>$filial, 'mdfe'=>$codigo, 'sending'=>!empty($manifesto->PROTOCOLO)];
            return $this->manifestoView($req, $data);
        }
    }

    // MANIFESTO - Enviar manifesto para a sefaz
    public function enviarManifesto(Request $req){
        $MDFe = new MDFe();
        $rt = ['status'=>'ERRO', 'msg'=>'Manifesto não encontrado', 'titulo'=>'Erro ao tentar enviar o MDFe', 'response'=>0];

        try{
            if(empty($req->filial) || empty($req->codigo)) throw new Exception('Filial e Código não informados!');
            $xml = $MDFe->montaMDFe($req->filial, $req->codigo);
            $sendClass = $MDFe->enviar();
            $send = $sendClass->toArray();

            $dup = ($send['cStat'] == '104' && $send['protMDFe']['infProt']['cStat'] == '204');
            if($send['status'] == 'OK' || $dup){
                if($dup) $MDFe->consultar(null, $std, $nSend);
                else $nSend = $send;

                $data = [
                    'ENVIO' => date('Y/m/d H:i:s'),
                    'AVERBACAO' => 'ENVIADO VIA WEB',
                    'PROTOCOLO' => $nSend['protMDFe']['infProt']['nProt'],
                ];

                $mdfe = DB::table('MANIFESTO')->where(['FILIAL'=>$req->filial, 'MANIFESTO'=>$req->codigo]);
                $mdfe->update($data);

                $sendClass->add('titulo', 'Envio de Manifesto');
                $sendClass->update('msg', $dup ? $send['msg'] : 'MDFe enviado com sucesso!');
                $sendClass->update('status', $dup ? 'ERRO' : 'OK');

                $sucess = $sendClass->toArray();
                $sucess['response'] = $mdfe->get()->toArray()[0] ?? null;
            } else {
                $sendClass->update('status', $send['status']);
                $sendClass->update('msg', $send['msg']);
            }

            $rt = $sucess ?? $sendClass->toArray();
        } catch (\Exception $e) {
            $rt['msg'] = $e->getMessage() . ' - ' . $e->getFile() . ' (' . $e->getLine() . ')';
        }
    ...
    // MANIFESTO - Cancela e envia o cancelamento para a SEFAZ
    public function cancelaManifesto(Request $req){
        $MDFe = new MDFe();
        $rt = ['status'=>'ERRO', 'msg'=>'Manifesto não encontrado', 'titulo'=>'Erro ao tentar enviar o MDFe', 'response'=>0];

        try{
            if(empty($req->filial) || empty($req->codigo)) throw new Exception('Filial e Código não informados!');
            else if(empty($req->motivo)) throw new Exception('Motivo não informado!');
            $xml = $MDFe->getMDFe($req->filial, $req->codigo);
            $send = $MDFe->cancelar($req->motivo)->toArray();

            if($send['status'] == 'OK'){
                $mdfe = DB::table('MANIFESTO')->where(['FILIAL'=>$req->filial, 'MANIFESTO'=>$req->codigo]);
                $mdfe->update(['SITUACAO'=>9, 'MOTIVO_CANC'=>$req->motivo, 'OPERADOR_CANC'=>Auth::user()->CODIGO, 'DATA_CANC'=>date('Y-m-d')]);
                $send['response'] = $mdfe->get()->toArray()[0] ?? [];
    ...
    // MANIFESTO - Imprime o manifesto
    public function imprimirManifesto(Request $req){
        $rt = ['status'=>'OK', 'msg'=>''];
        try{
            $MDFe = new MDFe();
            if($req->isMethod('post')){
                $MDFe->getMDFe($req->filial, $req->codigo);
                $MDFe->print();
            } else {
                $MDFe->getMDFe($req->filial, $req->codigo);
                return $rt;
    ...
    // MANIFESTO - Encerra e envia o manifesto para a SEFAZ
    public function encerrarManifesto(Request $req){
        $rt = ['status'=>'OK', 'msg'=>''];
        try{
            if(empty($req->filial) || empty($req->codigo)) throw new Exception('Filial e Código não informados!');
            else if(empty($req->date)) throw new Exception('Data de encerramento não informada!');
            $date = Format::format($req->date, 'date');

            $MDFe  = new MDFe;
            $xml = $MDFe->getMDFe($req->filial, $req->codigo, true);
            $send = $MDFe->encerrar($date)->toArray();

            if($send['status'] == 'OK'){
                $mdfe = DB::table('MANIFESTO')->where(['FILIAL'=>$req->filial, 'MANIFESTO'=>$req->codigo]);
                $mdfe->update(['SITUACAO'=>1, 'CHEGADA'=>$date]);
                $send['response'] = $mdfe->get()->toArray()[0] ?? [];
